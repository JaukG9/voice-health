...
def generate_summary(metrics: dict, trends: dict) -> str:
    if metrics.get("word_count", 0) < 5:
        return (
    ...
    if n_history == 0:
        return (
            ...
            "Future recordings will be compared against it to track how your "
            "speech changes over time."
        )
    if n_history < 3:
        return (
            f"Baseline recording {n_history + 1} of 3 complete. A few more "
            "recordings are needed before trends become meaningful — keep "
            "recording daily for the most reliable comparison."
        )

    score = trends["stability_score"]
    if score is None or score >= 85:
        opening = "Your speech is very consistent with your personal baseline."
    elif score >= 70:
        opening = "Your speech is broadly stable compared to your baseline, with some small variations."
    elif score >= 50:
